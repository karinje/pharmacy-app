...
function buildPackageOptimizerPrompt(drugName, totalQuantityNeeded, availablePackages, daysSupply) {
...
function buildExplanationPrompt(drugName, instructions, daysSupply, parsing, quantity, optimization) {
...
function parseJSONResponse(response) {
    try {
        ...
        throw new functions.https.HttpsError('internal', 'Failed to parse OpenAI response as JSON', { response, error: error instanceof Error ? error.message : String(error) });
    }
}
/**
 ...
 */
exports.calculatePrescription = functions
    .region('us-central1')
    .runWith({
    timeoutSeconds: 60,
    memory: '512MB'
    ...
    try {
        ...
        const parsing = parseJSONResponse(parsingCompletion.choices[0].message.content || '{}');
...
DO NOT include any text before or after the JSON object.`;
        ...
        const quantity = parseJSONResponse(outputText);
        // Step 3: Optimize packages with OpenAI
        const optimizationPrompt = buildPackageOptimizerPrompt(data.drugName, quantity.totalQuantityNeeded, data.availablePackages.map((p) => ({
            ndc: p.ndc,
            size: p.packageSize,
        ...
        throw new functions.https.HttpsError('internal', 'Failed to calculate prescription', error);
    }
});
