<script lang="ts">
	...
	let loading = false;
	let errorMessage = '';
	let errors: { email?: string; password?: string; confirmPassword?: string; displayName?: string } =
		{};
	...
	async function handleSubmit() {
		...
		try {
			// Validate form data
			const validated = signupSchema.parse({ email, password, confirmPassword, displayName });

			// Create account
			await authService.signup(validated.email, validated.password, validated.displayName);
